...
if "OVERVIEW" in page:
    ...
    if run_scan:
        ...
        # ── Scan all targets, store results in session_state ─────────────────
    ...
    # ── Display results from session_state (only renders selected target) ─────
    if "scan_results" in st.session_state and st.session_state["scan_results"]:
        ...
        # Styled radio buttons that look like tabs
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    try:
        ...
        if "level" in df.columns:
            ...
            st.dataframe(df.style.map(color_level_sr, subset=["level"]), use_container_width=True)
        else:
        ...
        for _row in history:
            _vt_rows.append({
                "IP":        _row.get("ip", "N/A"), "Score": _row.get("score", 0), "Level": _row.get("level", "N/A"),
                "Malicious": _row.get("malicious", "—"), "Suspicious":_row.get("suspicious", "—"), "Abuse": _row.get("abuse_score", "—"),
            })
        if _vt_rows: st.dataframe(pd.DataFrame(_vt_rows), use_container_width=True)
        else: st.markdown('<div class="terminal-box"><span class="t-yellow">[ INFO ]</span> No threat intel data stored yet.</div>', unsafe_allow_html=True)
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    try:
        ...
        if "level" in df.columns:
            ...
            st.dataframe(df.style.map(color_level_h, subset=["level"]), use_container_width=True)
        else:
            st.dataframe(df, use_container_width=True)
